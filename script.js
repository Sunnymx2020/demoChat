...
btnSignup.onclick = async () => {
  ...
  alert('Magic link sent. Verify and reopen.');
};
...
async function acceptRequest(r) {
  await supabase.from('contacts').insert([
    { user_uid: currentUser.id, contact_uid: r.from_uid },
    { user_uid: r.from_uid, contact_uid: currentUser.id }
  ]);
...
composer.addEventListener('submit', async (e) => {
  ...
  await supabase.from('messages').insert([{
    room: roomIdFor(currentUser.id, currentPeer),
